      ...
      <div class="container py-5" data-aos="fade-up">
		<div class="card">
              ...
              $status=$row["Status"];
							...
							echo "<a href='../../assets/programme/".$row["Image"]."' data-gallery='portfolioGallery' class='portfolio-lightbox preview-link' title='".$row["Name"]."'><img src='../../assets/programme/".$row["Image"]."' alt='Avatar' style='width:800px;height:auto'></a>
							<center><div style='width:800px;height:auto;' class='alert alert-info' role='alert'>Click on image to preview</div></center>
							<h5>".$row["Name"]."</h5>
							Fee:<p style='color:blue;'>RM" .$row["Fee Price"] ."</p>
							Venue:<p style='color:blue;'>" .$row["Venue"] ."</p>
							Date:<p style='color:blue;'>" .date("j F Y",strtotime($row["Date"])) ."</p>
							Status:<p style='color:blue;'>" .$row["Status"] ."</p>";
						}
					}
					$result1=mysqli_query($con,"SELECT * FROM `list` WHERE `Programme ID`='$_POST[id]' AND `Student`='$q'");
                ...
                $attendstat=$row["Attend Status"];	
							  ...
							  if($allow=="no"){
								...
								<form action='../Programme/register_process.php' method='post'>
									<button style='width:110px;margin:10px'class='btn btn-primary' $stat name='register'type='submit' value='$_POST[id]'><i class='fas fa-pen'></i> Register</button>
								</form>";
                    ...
                    if($attendstat=="Present"){
                      ...
                      <form action='../Programme/attend_process.php' method='post'>
                        <input type='hidden' name='programme' value='$_POST[id]'>
